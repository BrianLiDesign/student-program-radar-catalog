...
class GitHubScraper(EnhancedBaseScraper):
    ...
    def __init__(self, company_name: str, base_url: str):
        super().__init__(company_name, base_url, rate_limit_delay=2.0)  # Be extra careful with GitHub
    ...
    def parse_program_page(self, url: str) -> Optional[dict]:
        ...
        try:
            ...
            # Short description
            desc_elem = soup.find('p', class_='lead') or soup.find('div', class_='markdown-body').find('p') if soup.find('div', class_='markdown-body') else None
            if desc_elem:
                program_data['short_description'] = self._extract_text(desc_elem)
            else:
                program_data['short_description'] = "GitHub Campus Experts are student leaders who build technical communities on campus."
            ...
            program_data['eligibility_summary'] = "Students enrolled in a degree-granting institution"
            ...
            program_data['perks_detail'] = "Access to GitHub Enterprise, travel stipends for events, exclusive swag, mentorship from GitHub employees, access to GitHub Campus Program events"
            program_data['social_requirements'] = "Share learnings and experiences on social media using #GitHubCampusExpert"
            program_data['source_url'] = url
            program_data['source_snippet'] = "GitHub Campus Experts are student leaders who build technical communities on campus."
            program_data['school_restricted'] = False
            program_data['notes'] = "Applications typically open twice per year"

            # Generate UUID-like ID
            id_string = f"github{url}"
            program_data['id'] = "3ba33b4a-3c1a-5555-8e1a-" + hashlib.md5(id_string.encode()).hexdigest()[:12]
