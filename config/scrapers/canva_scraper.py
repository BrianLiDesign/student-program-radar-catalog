...
CANONICAL_PROGRAM_NAME = "Canva for Education"


class CanvaScraper(EnhancedBaseScraper):
    """Canva for Education scraper."""
    ...
    def parse_program_page(self, url: str) -> dict | None:
        if "canva.com/education" not in url:
            return None
        ...
        if "education" not in lower and "student" not in lower:
            logger.warning("Canva education page missing expected signals: %s", url)
            return None

        description = first_paragraph(soup) or (
            "Canva for Education helps students and teachers create and collaborate in the classroom."
        )
        ...
        return {
            ...
            "notes": "Parsed from canva.com/education landing page.",
        }
