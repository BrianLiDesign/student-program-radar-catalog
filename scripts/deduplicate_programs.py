...
def find_duplicates(programs, similarity_threshold=0.8):
    ...
    for i in range(len(programs)):
        for j in range(i + 1, len(programs)):
            is_dup, score, reason = are_programs_duplicate(programs[i], programs[j], similarity_threshold)
            if is_dup:
...
def select_canonical_program(programs, indices):
    ...
    def score_program(prog):
        ...
        required_fields = ['id', 'name', 'company', 'apply_url', 'status', 'role_type',
        ...
        optional_fields = ['responsibilities', 'time_commitment', 'perks_detail',
                          'deadlines', 'social_requirements', 'source_snippet']
...
def deduplicate_programs(programs, similarity_threshold=0.8):
    ...
    deduplicated_programs = [programs[i] for i in range(len(programs)) if i not in indices_to_remove]
...
def main():
    ...
    parser.add_argument('--output', '-o', default='data/active/programs_deduped.json',
                        help='Output JSON file for deduplicated programs')
    parser.add_argument('--duplicates', '-d', default='data/active/duplicates.json',
